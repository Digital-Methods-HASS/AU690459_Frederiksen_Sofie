...
show_bar <- ggplot(data = Western_Europe) + 
  geom_line(aes(x = Year, y = homicides_per_100k, color = Entity)) +
  labs(title = "homicide trends in Western Europe",
       x = "Year",
...
```{r eval = TRUE}
Western_Europe
show_bar + facet_wrap(~Entity, ncol = 2, nrow = 3)
...
```{r eval = FALSE}
kongerækken<- read_csv2(
  ("data/kongerækken.csv"), 
  na = "NULL") %>% 
    mutate(midyear = reignend - (reignend - reignstart)/2,
    reign_duration = reignend- reignstart)
midyear

ggplot(data = kongerækken) +
  geom_point(aes(x = midyear, y = reign_duration)) +
  geom_smooth(aes(x = midyear, y = reignduration), method = "lm") +
  labs(title = "Duration of Danish monarchs' reign",
       x = "Mid Year of Reign",
       y = "Duration of reign (years)") +
  scale_x_continuous(breaks = seq(900,2050,100))
  theme_classic()
  
```

Answer to what it tells me:
other than the outlier atthe beggining of the 1600 it tells me that monarch generally rule longer and som of the outliers might be due to the monarch being coronated as a child and therefore even if the monarch died at 30 they could still have "reigned" for 20 years
...
Whether or not we are more civilized today is really hard to say. It depends on what is defined at “civilized”. If we look at the murder rate, we seem to kill each other less which could make humans seem more civilized but at the same time our wars, especially in the 20th century, have become significantly deathlier, with bigger armies and corresponding bigger losses. 
It can also be argued that “civilization” as the West sees it, is not really civilized because of the way the West uses and abuses the shared resources of the planet, where the cost of upholding civilization right now seems to be the destruction of earth. But at least we don’t murder each other as much as in the dark medieval times.
